...
func GetRankList(c *gin.Context) {
	...
	err = models.DB.Model(new(models.UserBasic)).Count(&count).Order("finish_problem_num DESC, submit_num ASC").
		Offset(page).Limit(size).Find(&list).Error // Model指定查的表，Count查总共有多少条数据，Order进行排序（DESC-倒序-从大到小-降序，ASC-正序-从小到大-升序），Offset指定开始的位置，Limit指定每页数据的条数，Find将查到的数据映射到list上去
	if err != nil {
		c.JSON(http.StatusOK, gin.H{
			"code": -1,
